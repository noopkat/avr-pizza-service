...
Compiler.prototype.buildSketch = function(callback) {
  ...
  let builderChild = child.exec(`${self.paths.arduinoBuilder} ${builderString}`, function(error) {
    ...
    if (error) {
      ...
      // // get the important part out of the error message
      // // split on lines first, then pick the second line (just before the stack trace)
      // var errorline = error.message.split('\n')[1];
      // // split line into sections
      // var errorparts = errorline.split(' ');
      // // the errored file is the first part. Slice out only the file name and the line / col numbers
      // var errorfile = errorparts[0].split('/').slice(-1)[0];
      // // the error description is the rest of the error line
      // var errordesc = errorparts.slice(1).join(' ');
      // // join the error file and the description together
      // var errormsg = [errorfile, errordesc].join(' ');
      // on the second line, skip first part of the string, which is the server path.
      // return callback(errormsg);
      var errorline = error.message.split('/sketch/sketch.ino')[1];
