...
Compiler.prototype.prepareBuild = function(query, callback) {
  ...
  [self.paths.sketchBuild, self.paths.sketch, self.paths.sketchLibs].forEach(function(dir, index) {
...
Compiler.prototype.depositFiles = function(query, callback) {
  ...
  */
  ...
  if (query.libraries && Object.keys(query.libraries).length > 0) {
    ...
    Object.keys(libraries).forEach(function(key) {
      ...
      libraries[key].forEach(function(file) {
        mkdirp(path.join(self.paths.sketchLibs, key, file.filename.split('/').slice(0, -1).join('/')), function (error) {
          if (error) console.log('couldn\'t create tmp dir:', error);
          let ws = fs.createWriteStream(path.join(self.paths.sketchLibs, key, file.filename));
          ws.write(file.content);
          console.log('create a new library file in:', self.paths.sketchLibs, key, file.filename);
        });
      }) ;
    });
  }
  ...
  if (query.files && query.files.length) {
    ...
    console.log('didnt find any files.');
}
